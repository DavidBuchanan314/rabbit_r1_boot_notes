...
PATTERNS = [
	...
	Pattern(
		...
		alignment=2, # thumb instructions
		offset=0 # we're hooking this, not calling it
	),
	Pattern(
		name="lcd_printf",
	...
	Pattern(
		...
		offset=0 # we're also hooking rather than calling
	)
]

def test_patterns():
	lk_bin_dir = f"{SCRIPT_DIR}/../dumped_bins/lk_versions"

	for lk_name in sorted(os.listdir(lk_bin_dir), key=lambda x: x.split("-")[-1]):
		print()
		print(f"===== SCANNING {lk_name} =====")
		with open(f"{lk_bin_dir}/{lk_name}", "rb") as lk_file:
			lk_file.seek(0x200)
			data = lk_file.read().rstrip(b"\x00") # don't waste time scanning zeroes
		for pattern in PATTERNS:
			addr = find_pattern(data, pattern, LK_BASE)
			print(f"{pattern.name:<32} = {hex(addr)}")

def generate_headers(out_path: str):
	out = f"""\
/* GENERATED BY {__file__.split("/")[-1]} */
...
if __name__ == "__main__":
	test_patterns()
	generate_headers(SCRIPT_DIR + "/custom_da/patterns.h")
